...
export default function ProductCatalog() {
  ...
  const getStockBadge = (stock: number) => {
    if (stock <= 5) return <Badge variant="destructive">{t('products.lowStock')}</Badge>;
    if (stock <= 10) return <Badge variant="outline" className="border-yellow-500 text-yellow-600">{t('products.medium')}</Badge>;
    return <Badge variant="outline" className="border-green-500 text-green-600">{t('products.inStockBadge')}</Badge>;
  };
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
        ...
        <div className="flex gap-2 sm:gap-4">
          <Button variant="outline" size="sm" className="flex-1 sm:flex-none">{t('products.filterCategory')}</Button>
          <Button variant="outline" size="sm" className="flex-1 sm:flex-none">{t('products.filterMetal')}</Button>
        </div>
      </div>
            ...
            <CardContent className="pt-0 flex-1 flex flex-col">
              <div className="space-y-3 flex-1">
                <div className="grid grid-cols-1 gap-2 text-xs md:text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="font-medium text-muted-foreground">{t('products.metal')}:</span>
                    <span className="text-right">{product.metal}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium text-muted-foreground">{t('products.gemstone')}:</span>
                    <span className="text-right">{product.gemstone === 'None' ? t('common.none') : product.gemstone}</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="pt-3 border-t mt-auto">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-col gap-2">
                      <Button size="sm" variant="outline" className="w-full text-xs">{t('products.edit')}</Button>
                      <Button size="sm" className="w-full text-xs bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700">
                        {t('products.addToSale')}
                      </Button>
                    </div>
                  </div>
                </div>
